...
feature_selection = function(learner, task, strategy){
  ...
  sf = selectFeatures(learner, task, rdesc, control = control, show.info = TRUE)
...
model_validation_classif = function(data, learnerName, selected_features, sigma = NA){
  set.seed(42)
  learnerAlgo = createLearner(data, learnerName, selected_features)
...
```{r Plot classification model results}
predictions_classif = readRDS("predictions_classif.RData")
selectedFeatures_classif = readRDS("selectedFeatures_classif.RData")

# Create the base plot
ERT_Model1 = predictions_classif$`classif.rpart _ sffs` %>% filter(dim == 3)
ERT_SBS = data %>% filter(dim == 3)

df <- data.frame(
  Model1 = ERT_Model1$relERT,
  SBS = ERT_SBS$HCMA,
  FID = as.factor(ERT_Model1$fid)
)
sorted_fids = order(levels(df$FID))
df$FID <- factor(df$FID, levels = sorted_fids)

base_plot <- ggplot(df, aes(x = Model1, y = SBS, color = FID)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10() + 
...
# Create a facet grid of plots
final_plot <- base_plot + 
  facet_grid(Model ~ Condition) +
  theme(legend.position = "bottom") +
  guides(color = guide_legend(title = "BBOB-Group"), shape = guide_legend(title = "Function ID"))

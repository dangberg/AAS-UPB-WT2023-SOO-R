...
for(learner in learners)
{
  ...
  for(solverA in 1:(length(solver_names)-1)){
    for(solverB in (solverA+1):length(solver_names)){
      ...
      name = paste(learner, solverAName, solverBName)
      
      data_temp = data %>% 
        select(all_of(feature_names), solverAName, solverBName) %>%
        mutate(solverDiff := .data[[solverAName]] - .data[[solverBName]]) %>%
        select(-solverAName, -solverBName)
      regr.task = makeRegrTask(id = name, data = data_temp, target = "solverDiff")
      model = train(regr.lrn, regr.task)
      pair_regr_models[[name]] = model
    }
  }
}
```
...
model_validation_regr = function(data, learner){
  ...
  for(index in 1:nrow(data)){
    ...
    for(solver in solver_names){
        data_train = data[-index, ] %>% select(all_of(feature_names), solver)
        learnerAlgo = makeLearner(learner)

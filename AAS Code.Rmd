...
for(learnerName in learners){
  for(strategy in strategies){
    ...
    task = makeRegrTask(data = data_train, target = "solverDiff")
    sel_features = feature_selection(learner, task, strategy)
    predictions_pair_regr[[name]] = sel_features
...
relERT = read_csv("relERT.csv")
predictions_classif = readRDS("predictions_classif.RData") # TODO: For all types of models
selectedFeatures_classif = readRDS("selectedFeatures_classif.RData") # TODO: For all types of models

# select data for plotting
model = predictions_classif$`classif.rpart _ sffs` # TODO: For every model
sbs = relERT %>% select(fid, dim, HCMA)

# create factors for plotting
df = merge(model, sbs, by=c("dim","fid"))
df$dim = as.factor(df$dim)
sorted_dims = order(levels(df$dim))
df$dim = factor(df$dim, levels = sorted_dims)
df$fid = as.factor(df$fid)
sorted_fids = order(levels(df$fid))
df$fid = factor(df$fid, levels = sorted_fids)
df = df %>% arrange(dim, fid)

# print performance data
print(paste0("relERT model: ", mean(df$relERT)))
print(paste0("relERT model with FC: ", mean(df$relERT_FC)))
print(paste0("relERT SBS (HCMA): ", mean(df$HCMA)))

# TODO: different plots per dimension
ggplot(df, aes(x = relERT, y = HCMA, color = fid)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10() + 
...
ggplot(df, aes(x = relERT_FC, y = HCMA, color = fid)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10() + 
  geom_abline(slope = 1, intercept = 0, color = "grey80", linetype = "dashed") + 
...
```{r}
a = aes(x=as.numeric(fid), y=relERT)
b = aes(x=as.numeric(fid), y=HCMA)

df %>% filter(dim == 2) %>%
ggplot() +
  scale_x_continuous(limits = c(1,24), expand = c(0, 0.1)) +
  scale_y_log10() +
  geom_line(a, color = "red")+
  geom_point(a)+
  geom_line(b, color = "green")+
  geom_point(b) + 
  geom_ribbon(aes(x=as.numeric(fid), ymax=relERT, ymin=HCMA), fill="grey", alpha=.5)
  
```

```{r}
library(ggallin)
test = df %>% filter(dim == 2) %>% mutate(diff = HCMA - relERT)

ggplot(test, aes(x=fid, y=diff)) +
  #scale_y_log10() +
  geom_col() +

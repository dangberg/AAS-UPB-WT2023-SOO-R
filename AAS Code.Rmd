...
data = merge(features,relERT,by=c("dim","fid")) %>% arrange(dim, fid)
```
...
classif.task = makeClassifTask(data = data_train, target = "Best")
...
model_validation_classif = function(data, learner, selected_features){
  ...
  for(index in 1:nrow(data)){
    ...
    row = c(dim = data_val$dim, fid = data_val$fid, algo = predictedAlgo, relERT = predictionRelERT)
...
set.seed(42)
predicted_relERT = model_validation_classif(data, "classif.ksvm", sel_features)

# calculate mean relERT
predicted_relERT$relERT = as.numeric(predicted_relERT$relERT)
print(mean(predicted_relERT$relERT))

#learners = c("classif.rpart", "classif.ksvm", "classif.randomForest", "classif.xgboost")
#for(learner in learners){
#  predicted_relERT = model_validation_classif(data, learner, feature_names)
...
set.seed(42)
predicted_relERT = model_validation_regr(data, "regr.randomForest", sel_features)
  # calculate mean relERT
predicted_relERT$relERT = as.numeric(predicted_relERT$relERT)
print(mean(predicted_relERT$relERT))

filtered_pred = predicted_relERT %>% filter(dim != 3)
print(mean(filtered_pred$relERT))
#learners = c("regr.rpart", "regr.ksvm", "regr.randomForest", "regr.xgboost", "regr.earth")
#for(learner in learners){
#  predicted_relERT = model_validation_regr(data, learner, feature_names)
#  print(paste0(learner, " : ", predicted_relERT))
#}
```
